        ...
        <div id="inventoryTableWrapper" class="table-responsive border mb-1">
            <table class="table table-striped table-hover">
                ...
                <tbody>
                    ...
                    @if($item->amount < $item->par)
                    <tr class="border underParBorder underParBg">
                    @else
                    <tr class="border">
                    @endif
                            ...
                            @if($item->logistics_group_id == 0 || $item->logistics_group_id == null)
                                <td><a class="text-danger" href="{{ config('baseUrl') }}/inventory/{{$item->id}}/edit">Assign</a></td>
                            @else
                                <td><a href="{{config('baseUrl')}}/logistics/{{$item->logistics_group_id}}">{{$item->logistics_group_name}}</a></td>
                            @endif
                            <td class="textTruncate">{{$item->current_location}}</td>
